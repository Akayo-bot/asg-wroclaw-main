...
const AnimatedLogoutButton: React.FC<AnimatedLogoutButtonProps> = ({ onClick }) => {
    const { t } = useI18n();
    ...
    useEffect(() => {
        ...
        const handleClick = () => {
            if (stateRef.current === 'default' || stateRef.current === 'hover') {
    ...
    return (
        <button
            ...
            tabIndex={0}
            aria-label={t('auth.logout', 'Вийти')}
            data-logout-button
        >
            <svg className="doorway" viewBox="0 0 100 100" style={{ pointerEvents: 'none' }}>
            ...
            <svg className="figure" viewBox="0 0 100 100" style={{ pointerEvents: 'none' }}>
            ...
            <svg className="door" viewBox="0 0 100 100" style={{ pointerEvents: 'none' }}>
                <path d="M93.4 86.3H58.6c-1.9 0-3.4-1.5-3.4-3.4V17.1c0-1.9 1.5-3.4 3.4-3.4h34.8c1.9 0 3.4 1.5 3.4 3.4v65.8c0 1.9-1.5 3.4-3.4 3.4z" />
                <circle cx="66" cy="50" r="3.7" />
            </svg>
            <span className="button-text" style={{ pointerEvents: 'none' }}>{t('auth.logout', 'Log Out')}</span>
        </button>
    );
};
